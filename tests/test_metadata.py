...
def test_data_type_metadata_config_prejoin_records_default_true():
    config = DataTypeMetadataConfig(
        ...
        data_path_segment="data/example",
    )
...
def test_data_source_metadata_configs_are_derived_from_base_class():
    ...
    assert (
        TIMDEXRecords.METADATA_CONFIG.data_path_segment == TIMDEXRecords.DATA_PATH_SEGMENT
    )
    ...
    assert (
        TIMDEXEmbeddings.METADATA_CONFIG.data_path_segment
        == TIMDEXEmbeddings.DATA_PATH_SEGMENT
    )
